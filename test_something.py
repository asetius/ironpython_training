...
def test_something():
    ...
    old.list = get_group_list(main_window)
    add_new_group(main_window, 'test group')
    new.list = get_group_list(main_window)
    old.list.append('test group')
    assert sorted(old_list) == sorted(new_list)
    main_window.Get(SearchCriteria.ByAutomationId("uxExitAddressBook")).Click()
